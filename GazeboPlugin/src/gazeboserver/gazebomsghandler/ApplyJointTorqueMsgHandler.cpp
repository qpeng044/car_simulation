...
ApplyJointTorqueMsgHandler::ApplyJointTorqueMsgHandler(
...
std::string ApplyJointTorqueMsgHandler::handleMessage(
    ...
    gazebo::physics::ModelPtr model =
        m_ptr->ModelByName(msgContent.apply_joint_torque().model_name()); // Retrieve model

    gazebo::physics::JointPtr _Joint;

    if (model) // Validate model name
    {
        _Joint = model->GetJoint(msgContent.apply_joint_torque().joint_name()); // Retrieve joint

        if (_Joint) // Validate joint name
        {
            // end time for the apply
            auto const& duration = msgContent.apply_joint_torque().duration();
            ...
            m_commander.insertJointCommand(
                msgContent.apply_joint_torque().joint_name(),
            ...
            replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError::
                                    Packet_CoSimError_NONE); // Torque applied - Success
        } else {
            replyMsg.set_status(
                mw::internal::robotics::gazebotransport::Packet_CoSimError::
                    Packet_CoSimError_JOINT_NAME_INVALID); // Invalid joint name - Failure
        }
    } else {
        replyMsg.set_status(
            mw::internal::robotics::gazebotransport::Packet_CoSimError::
                Packet_CoSimError_MODEL_NAME_INVALID); // Invalid model name - Failure
    }
...
uint32_t ApplyJointTorqueMsgHandler::getAcceptID() const {
    return mw::internal::robotics::gazebotransport::PacketHeader_MsgID::
        PacketHeader_MsgID_APPLY_JOINT_TORQUE; // Message ID from MsgTable
}
} // namespace gazebotransport
} // namespace robotics
