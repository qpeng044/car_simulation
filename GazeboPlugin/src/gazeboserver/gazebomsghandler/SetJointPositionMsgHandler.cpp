...
SetJointPositionMsgHandler::SetJointPositionMsgHandler(
...
std::string SetJointPositionMsgHandler::handleMessage(
    ...
    gazebo::physics::ModelPtr model =
        m_ptr->ModelByName(msgContent.set_joint_position().model_name()); // Retrieve model

    if (model) // Validate model name
    {
        gazebo::physics::JointPtr _Joint =
            model->GetJoint(msgContent.set_joint_position().joint_name()); // Retrieve joint

        if (_Joint) // Validate joint name
        {
            // end time for the set joint position
            auto const& duration = msgContent.set_joint_position().duration();
            ...
            m_commander.insertJointSetPositionCommand(
                msgContent.set_joint_position().joint_name(),
            ...
            replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError::
                                    Packet_CoSimError_NONE); // Set Joint Position - Success
        } else {
            replyMsg.set_status(mw::internal::robotics::gazebotransport::Packet_CoSimError::
...
uint32_t SetJointPositionMsgHandler::getAcceptID() const {
    return mw::internal::robotics::gazebotransport::PacketHeader_MsgID::
        PacketHeader_MsgID_SET_JOINT_POSITION; // Message ID from MsgTable
}
} // namespace gazebotransport
} // namespace robotics
